...
Dockerとは「環境を簡単に構築するツール」です。いちいちいろんなアプリをインスコせずとも、仮想環境を作る⇒いらなくなったらポイ。で(ローカル)環境にも優しい！<br><br>
この記事の対象は以下の人達です。<br>
<br>
...
<br>
このDockerfileのおかげで「パッケージが充実しまくったRstudioがブラウザ上で利用」できます。すごい。<br>
この記事では「パッケージが充実しまくったRstudioがブラウザ上で利用できる」ことを目指します。「詳しい使い方の前にまぁ慣れようや」がコンセプトです。<br><br>
ゴールまでのプロセスは以下の通り。<br>
...
2. Docker Toolboxでいく(参照：[公式Doc](https://docs.docker.com/toolbox/toolbox_install_windows/)[exeファイル](https://github.com/docker/toolbox/releases))
  ...
  2の場合のプロセスは以下の通り：タスクマネージャ⇒パフォーマンス⇒仮想化が有効か確認⇒[https://github.com/docker/toolbox/releases](https://github.com/docker/toolbox/releases)のexeファイルをDLして起動⇒なすがまま⇒[Docker Quickstart terminal]をつける⇒待つ⇒くじらが出てくる＋[docker run hello-world]と入力して特にエラーがなければおｋ<br>
...
```docker:run
docker run -e PASSWARD=任意のぱすわーど -p 8787:8787 -v 絶対パス(例：//c/user/hoge/Documents/test)://home/rstudio -d --name 任意のコンテナ名 ykunisato/paper-r-jp
```  
<br>
これをしてしばらく待つ。「-v 'path'」には仮想マシンとやりとりするためのファイルが入った(or入る予定の)フォルダを用意しておくと、ローカルの環境と仮想環境でやりとりできる。<br>
※"/"が常識よりも先頭に一個多いのは僕のノートPCのWindows環境(Toolbox, Gitbash)ではこうしないとうまく動かなかったためです。うまくいかないのは大体このパス関連が失敗してるのでいろいろ調べたりしてみてください
<br>
<br>
...
<br>
IDにrstudio, passに上で書いた任意のパスワードを入力する。<br>
<br>
